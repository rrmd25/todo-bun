...
const createTable = "CREATE TABLE IF NOT EXISTS todo ( id INTEGER PRIMARY KEY AUTOINCREMENT, task VARCHAR(256) NOT NULL, description VARCHAR(512), author VARCHAR(64) NOT NULL, created_at DATE NOT NULL, last_update DATE NOT NULL);"
...
const insertTodo = "INSERT INTO todo (task, description, author, created_at, last_update) VALUES ($task, $description, $author, $createdAt, $lastUpdate) RETURNING *;"
const insertTodoQuery = db.prepare(insertTodo)

const updateTodo = "UPDATE todo SET task=$task, description=$description, author=$author, last_update=$lastUpdate WHERE id=$id RETURNING *;"
